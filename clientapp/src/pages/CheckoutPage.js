...
function CheckoutPage({ API_BASE_URL }) {
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      // ИЗМЕНЕНО: Проверяем, что статус находится в диапазоне 2xx (успех), а не строго 200.
      // Axios автоматически обрабатывает 2xx статусы как успешные, если только response.status не 204.
      // Для 201 Created, response.data будет содержать ответ от сервера.
      if (response.status >= 200 && response.status < 300) { // Проверяем успешный диапазон статусов
        ...
        setSelectedItems([]); 
        setTimeout(() => {
          navigate('/orders'); // Перенаправить на страницу истории заказов
        }, 2000);
      } else {
        // Этот блок будет выполнен только для неуспешных 3xx, 4xx, 5xx статусов
